...
internal static class Conversions
{
    public static string EffectiveTypeFace(this Word.RunProperties runProperties, IReadOnlyCollection<Word.StyleRunProperties> styleRuns, string defaultTypeFace)
    {
        ...
        string? typeFace = effectiveRunFonts?.Ascii;
        ...
        LineAlignment ifNull)
    {
        ...
        if(justification.Val.Value == Word.JustificationValues.Left) return LineAlignment.Left;
        if(justification.Val.Value == Word.JustificationValues.Center) return LineAlignment.Center;
        if(justification.Val.Value == Word.JustificationValues.Right) return LineAlignment.Right;
    ...
    {
        Word.LineSpacingRuleValues rule = spacingBetweenLines.LineRule?.Value ?? Word.LineSpacingRuleValues.Auto;
    ...
    private static LineSpacing GetLineSpacing(this StringValue? line, EnumValue<Word.LineSpacingRuleValues> lineRule)
    {
        ...
        LineSpacing lineSpacing = rule.ToLineSpacing(line);
        return lineSpacing;
    }

    private static LineSpacing ToLineSpacing(this Word.LineSpacingRuleValues rule, StringValue? line)
    {
        LineSpacing lineSpacing = new AutoLineSpacing();
        if (rule == Word.LineSpacingRuleValues.Auto) lineSpacing = new AutoLineSpacing(line?.ToLong() ?? AutoLineSpacing.Default);
        if (rule == Word.LineSpacingRuleValues.Exact) lineSpacing = new ExactLineSpacing(line.ToPoint());
        if (rule == Word.LineSpacingRuleValues.AtLeast) lineSpacing = new AtLeastLineSpacing(line.ToPoint());
        return lineSpacing;
    }
}
